...
long <- raw %>%
  ...
  mutate(
    Ointment = factor(Ointment, levels = c('a','b','c','d' ))
  )
list(raw$Ointment)
...
baseline <- data.frame(Ointment=c('a','b','c','d'),
                       Baseline=300)
...
tukey_clean <- tukey_res %>%
  mutate(
    ...
    prefix = case_when(
      str_detect(comp, fixed('a'))            ~ "a",
      str_detect(comp, fixed('b'))        ~ "b",
      str_detect(comp, fixed('c'))       ~ "c",
      TRUE ~ ""
    ),
    sup = ifelse(symbol == "" | prefix == "", "", paste0(prefix, symbol))
  )

tukey_long <- tukey_clean %>%
  pivot_longer(c(group1, group2), names_to = "side", values_to = "Ointment") %>% 
  filter(side != "group1") %>% 
  select(Days, Ointment, sup)
...
final_stats <- summary_stats %>%
  ...
  left_join(tukey_long, by = c("Days", "Ointment")) %>%
  mutate(
...
plos_table <- final_stats %>%
  ...
  select(Days, Ointment, formatted) %>%
  ...
  rename(
    `Simple Ointment`=a,
    `5%(w/ w) Extract ointment`=b,
    `10%(w /w) Extract ointment`=c,
    `0.5% NS+B (w/ w) Ointment`=d
  ) %>%
  # Step 5: Reorder columns
  select(
    Days,
    `Simple Ointment`,
    `5%(w/ w) Extract ointment`,
    `10%(w /w) Extract ointment`,
...
export(plos_table, "WoundHealing_excision_extract_PLOS_Table.xlsx")
