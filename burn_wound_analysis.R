...
# 1. Input data
# ================================
raw <- wound_raw_burn_data  # your raw table
...
long <- raw %>%
  ...
  mutate(
    Ointment = factor(Ointment, levels = c('a_','b_','c_','d_' ))
  )
list(raw$Ointment)
...
baseline <- data.frame(Ointment=c('a_','b_','c_','d_'),
                       Baseline=300)
...
tukey_clean <- tukey_res %>%
  mutate(
    ...
    prefix = case_when(
      str_detect(comp, fixed('a_'))            ~ "a",
      str_detect(comp, fixed('b_'))        ~ "b",
      str_detect(comp, fixed('c_'))       ~ "c",
      TRUE ~ ""
    ),
...
final_stats <- summary_stats %>%
  ...
  left_join(tukey_long, by = c("Day", "Ointment")) %>%
  mutate(
...
plos_table <- final_stats %>%
  ...
  rename(
    `Simple Ointment`=a_,
    `5%(w/ w) Extract ointment`=b_,
    `10%(w /w) Extract ointment`=c_,
    `0.5% NS+B (w/ w) Ointment`=d_
  ) %>%
...
export(plos_table, "WoundHealing_burn_PLOS_Table.xlsx")
